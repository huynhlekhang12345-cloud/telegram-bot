...
# --- KHỞI TẠO CƠ SỞ DỮ LIỆU SQLITE TOÀN DIỆN (TỐI ƯU INDEX) ---
def init_db():
...
def check_rate_limit(user_id, cooldown_seconds=10):
    now = time.time()
...
# ==========================================
# --- THEO DÕI TIN NHẮN, USER & FILE (SAFE) ---
# ==========================================
LAST_UPLOADED_LINES = []

@client.on(events.NewMessage)
async def track_all_users_and_files(event):
    global LAST_UPLOADED_LINES
    if event.sender_id:
        try:
            conn = sqlite3.connect('bot_database.db', timeout=10.0)
            cursor = conn.cursor()
...
def is_authorized(user_id):
    ...
    try:
        ...
        if row:
            ...
            # Tặng dùng thử 3 ngày cho user mới
...
async def help_menu_handler(event):
    ...
    help_text = (
        "🤖 **HỆ THỐNG BOT QUẢN LÝ & TIỆN ÍCH (24/7)** 🤖\n"
        "----------------------------------------\n"
...
# ==========================================
# --- QUẢN LÝ KHO BOT & KIỂM TRA SỐNG/CHẾT (`.checkbot`) ---
# ==========================================
...
async def check_bot_status(event):
    if not is_authorized(event.sender_id): return
    msg = await event.respond("⏳ Đang quét kiểm tra sống/chết toàn bộ bot trong kho...", parse_mode='markdown')
    ...
    try:
        ...
        for b_id, token in bots:
            try:
                ...
                if res.get("ok"):
                    cursor.execute("UPDATE bot_kho SET status = 'ACTIVE' WHERE id = ?", (b_id,))
                    active_count += 1
                else:
                    cursor.execute("UPDATE bot_kho SET status = 'DEAD' WHERE id = ?", (b_id,))
                    dead_count += 1
            except Exception:
                cursor.execute("UPDATE bot_kho SET status = 'DEAD' WHERE id = ?", (b_id,))
                dead_count += 1
...
# ==========================================
# --- HỆ THỐNG SPAM, TOXIC & TREO NGÔN (CÓ RATE LIMIT) ---
# ==========================================
async def execute_task_with_custom_bots(chat_id, num_bots, delay, event_msg, command_name):
    global LAST_UPLOADED_LINES
    
    # Kiểm tra chống spam lệnh (Cooldown 10 giây)
    remaining = check_rate_limit(event_msg.sender_id, cooldown_seconds=10)
    if remaining > 0:
    ...
    if not content_lines:
        content_lines = LAST_UPLOADED_LINES

    if not content_lines:
        await event_msg.respond("⚠️ Không tìm thấy nội dung văn bản hoặc file `.txt` nào!", parse_mode='markdown')
        return
    ...
    if sub_tokens:
        ...
        await send_log_to_boxtong(f"⚡ **TIẾN TRÌNH HOẠT ĐỘNG:** User `{event_msg.sender_id}` chạy `.{command_name}` dùng `{len(sub_tokens)}` bot phụ tại Chat ID: `{chat_id}`.")
    else:
...
async def run_sub_worker(token, chat_id, lines, delay, idx, task_id):
    try:
        sub = TelegramClient(f'sub_worker_{task_id}_{idx}_{random.randint(1000,9999)}', API_ID, API_HASH)
...
def main():
    keep_alive()
    print("🤖 Bot System is starting and running 24/7 with advanced features...")
    
    # Khởi chạy luồng quét key hết hạn ngầm
