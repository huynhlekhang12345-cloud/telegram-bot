...
# Chạy Web Server ngầm để vượt lỗi "No open ports detected" trên Render
...
async def start_command(event):
    ...
    text = (
        ...
        f"----------------------------------------\n"
        f"👤 **Cấp bậc:** `{role}`\n"
        f"📢 **Box Tổng hiện tại:** `{box_tong if box_tong else 'Chưa thiết lập (Dùng .setboxtong)'}`\n"
...
async def set_box_tong_command(event):
    ...
    if not is_admin(user_id):
        await event.respond("⚠️ Chỉ có Admin mới có quyền cài đặt Box Tổng!", parse_mode='markdown')
        return
    ...
    try:
        with sqlite3.connect('bot_database.db', timeout=5.0) as conn:
            ...
            conn.commit()
        await event.respond(f"✅ **Đã cập nhật Box Tổng thành công!**\n• ID Box nhận thông báo: `{target_box}`", parse_mode='markdown')
    except Exception as e:
        await event.respond(f"❌ Lỗi lưu Box Tổng: {e}", parse_mode='markdown')
