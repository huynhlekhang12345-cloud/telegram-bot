...
ADMIN_ID = 8725740462         # Đã cập nhật ID Admin chính chủ của bạn
QUAN_TRIVIEN = set()          
NGUOI_THUE = set()            
...
# --- LỆNH /STOP: DỪNG TẤT CẢ TIẾN TRÌNH ĐANG TREO/SPAM/TOXIC TRONG CHAT ---
...
def upload_permanent_file(message):
    ...
    if message.document:
        ...
        target_doc = message.reply_to_message.document

    if not target_doc:
        bot.reply_to(message, "⚠️ Vui lòng gửi file kèm chữ `/upfile`, hoặc reply vào tin nhắn có file kèm lệnh `/upfile`!", parse_mode="Markdown")
        return
